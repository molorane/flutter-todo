...
class SortObject {
  ...
  @override
  String toString() =>
      'SortObject[empty=$empty, unsorted=$unsorted, sorted=$sorted]';
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "SortObject[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "SortObject[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<SortObject>? listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
  ...
  static Map<String, List<SortObject>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = SortObject.listFromJson(
          entry.value,
          growable: growable,
        );
        if (value != null) {
